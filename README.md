...
В `.env` — те же переменные, что и для OpenRouter:
...
`OPEN_API_KEY` для Ollama не нужен.

### Добавление моделей

```powershell
ollama pull llama3.2:1b   # текст (MODEL, AUDIO_MODEL)
ollama pull llava:7b      # фото (VISION_MODEL)
...
ollama rm llava:7b
ollama rm llama3.2:1b llava:7b
```

Удалить все модели:

```powershell
ollama list --format "{{.Name}}" | ForEach-Object { ollama rm $_ }
```

Имена — как в выводе `ollama list` (например `llama3.2:1b`).
